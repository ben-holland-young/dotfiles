...
#sep = command.split(" ")
vocab = {"visual": "v", "change": "c", "delete": "d", "yank" : "y", "copy" : "y", "paste": "p", "inside" : "i", "around" : "a", "till" : "t", "word" : "w", "sentence" : "s", "paragraph" : "p", "block" : "b", "tag" : "t", "quotes": "\"", "brackets" : "(", "go to line" : ":", "back to top" : "gg", "bottom" : "G", "start of line" : "0", "delete line" : "dd", "line above" : "O", "line below" : "o", "nerdtree": "§n"}
...
"math vim
function! MathVi()
let operation = input("Operation: derive, factor, simplify, integrate, find tangent eqtn, area under curve, cos, sin, tan: ")
let val = input("Operate on: ")
python << EOF
import vim, urllib2, json
question = vim.eval("operation")
value = vim.eval("val")
send_url = 'https://newton.now.sh/'+question+'/'+value
response = urllib2.urlopen(send_url, None, 20).read()
j = json.loads(response)
result = j.get("result")
vim.command('vnew | r ! echo %s' % result)
EOF
endfunction
"map control m to MathVi
map <C-m> :call MathVi()<cr>
"switch to terminal quickly
map == :sh<cr>
